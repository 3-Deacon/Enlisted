...
- **Five-plan integration roadmap (2026-04-21)** — the Spec 2 Orders Surface remainder and the Campaign Intelligence Backbone were merged into one integrated loop. Integration spec: [docs/superpowers/specs/2026-04-21-plans-integration-design.md](docs/superpowers/specs/2026-04-21-plans-integration-design.md). Design spec (truth layer): [docs/superpowers/specs/2026-04-21-enlisted-campaign-intelligence-design.md](docs/superpowers/specs/2026-04-21-enlisted-campaign-intelligence-design.md).
  ...
  - **Plan 2** — Lord AI Intervention (model wrappers, snapshot-driven). Archived plan at [docs/superpowers/plans/archive/2026-04-21-lord-ai-intervention.md](docs/superpowers/plans/archive/2026-04-21-lord-ai-intervention.md). ✅ Shipped on `development` 2026-04-22 (commits `4929246` → `f289b9b`). Three MBGameModel wrappers registered via `SubModule.OnGameStart` bias target choice, army formation, and pursuit for the enlisted lord only; every override gated per-call on `EnlistedAiGate.TryGetSnapshotForParty` + falls through to vanilla `BaseModel`. Phase H smoke passed 2026-04-22 — load-bearing T13 bait-break observed in-game (score 276→0, lord pivoted `OffensiveSiege` → `FrontierDefense`). Phase F narrow Harmony not required. Log excerpt: [docs/Features/CampaignIntelligence/plan2-phase-h-log.md](docs/Features/CampaignIntelligence/plan2-phase-h-log.md).
  ...
  - **Plan 4** — Duty Opportunities (`DutyOpportunityBuilder` generates ambient duty storylets; absorbs Orders Tasks 27-32 profile pools + Task 35 transitions + validator Phase 13). Plan at [docs/superpowers/plans/2026-04-21-duty-opportunities.md](docs/superpowers/plans/2026-04-21-duty-opportunities.md). ✅ Shipped on `development` 2026-04-22 (commits `ca22111` → `0c519d3`, 21 commits + status closure = 22). Ambient episodic + arc-scale Modal emissions gated per-tick on `EnlistedAiGate`-equivalent `IsEnlisted` + snapshot + `OrderActivity.Instance`; transition-interrupt hook inside `DutyProfileBehavior.CommitTransition` closes committed arcs on profile flip via three-tier fallback chain. Four new snapshot-backed `TriggerRegistry` predicates + `DutyCooldownStore` at save-definer offset 50. 140 authored storylets across 7 ambient pools + transition.json. Phase 13 pool-coverage validator warnings-only. Verification doc: [docs/superpowers/plans/2026-04-21-duty-opportunities-verification.md](docs/superpowers/plans/2026-04-21-duty-opportunities-verification.md). T26 + T27 in-game smoke pending human operator.
  - **Plan 5** — Career Loop Closure (path crossroads + culture/trait overlays + debug hotkeys + save-load + final playtest). Plan at [docs/superpowers/plans/2026-04-21-career-loop-closure.md](docs/superpowers/plans/2026-04-21-career-loop-closure.md). Verification doc: [docs/superpowers/plans/2026-04-21-career-loop-verification.md](docs/superpowers/plans/2026-04-21-career-loop-verification.md). Playtest runbook: [docs/superpowers/plans/2026-04-21-career-loop-playtest-scenarios.md](docs/superpowers/plans/2026-04-21-career-loop-playtest-scenarios.md). Living day-to-day reference: [docs/Features/Content/career-loop.md](docs/Features/Content/career-loop.md). 🟡 **Code-level verification complete; smoke pending.** Tasks T1-T9 (Half A) + T11-T16 + T18 + T22 (Half B code + docs) shipped on `development` 2026-04-22 (commits `4f66604` → `b517909`, 21 commits; pushed to `origin/development`). Ships the full path crossroads system (`commit_path` / `resist_path` primitives, `QualityStore.SetDirect` back-door, `PathIds` helper, `PathScorer.BumpPath` resist bias, `PathCrossroadsBehavior`), 15 milestone storylets + 30 T7+ committed-path variants (scout+escort × 5 paths; minimum-viable scope), 45 culture overlays (3 cultures × 15 hot-path bases with emitter gate + overlay preference in `EnlistedDutyEmitterBehavior`), 15 big-5 trait-gated storylets (emitter trait gate), `CareerDebugHotkeysBehavior` (Ctrl+Shift+I/A/O/F), `validate_content.py` Phase 15 full enforcement + Phase 13 overlay-skip, and the seven-scenario playtest runbook. **Pending human operator:** T10 (Half A crossroads smoke), T17 (save-load mid-arc reconstruction), T19-T21 (scenarios A-G execution). **Deferred to optional Half B polish:** T7 archetype expansion beyond scout+escort (~40 variants), culture-overlay expansion to the remaining 3 cultures per base (~45 overlays), Signal emitter culture/trait gate (contingent on new signal content). Plan 5 stays 🟡 until human smoke closes; v2 redesign is NOT claimed complete on this commit.
- **Integrated smoke coverage (2026-04-22).** One human playtest run of Scenarios A-H in [docs/superpowers/plans/2026-04-21-career-loop-playtest-scenarios.md](docs/superpowers/plans/2026-04-21-career-loop-playtest-scenarios.md) covers the pending human-operator smoke queue across Plans 1, 3, 4, and 5 (Plan 2's Phase H smoke passed 2026-04-22). "Covers" means the scenarios exercise the behaviour each pending task wants to verify, not that the run auto-closes those tasks — a runner should still read each plan's verification doc and confirm the specific assertions there. See the cross-plan coverage matrix in [docs/superpowers/plans/2026-04-21-career-loop-verification.md](docs/superpowers/plans/2026-04-21-career-loop-verification.md) §6.
- **Specs 3–5** — Land/Sea, Promotion+Muster, Quartermaster — not yet started; save-definer class offset range 51-60 reserved (offsets 48 + 49 + 50 claimed by Plans 1 + 3 + 4 per AGENTS.md Rule #11).
