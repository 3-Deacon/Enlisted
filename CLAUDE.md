...
- **Spec 2 (Orders Surface)** — Phase A code complete (Tasks 1-17 on `development`, commits `2f64137` → `b8039cf`; save-definer offsets 46/47/85 claimed; in-game smoke verified — PATH / DUTYPROFILE / DRIFT heartbeats). Phase B §13.1 `OrderManager` → `OrderDisplayHelper` migrations done (13 consumer sites + Task 19f deferred ShowOrdersMenu, `0eed684`). Phase B content started: Task 25 (`duty_profiles.json`, `85d8b35`) and Task 26 (`LootPoolResolver` + `loot_pools.json` + EffectExecutor rewire, `90bef6a`; see API corrections appendix). Phase B validator rails shipped: Tasks 37-40 added validator phases 14-17 (arc integrity, crossroads stub, loot pool sanity, per-option rate caps + `committed_path` read-only), commits `55e7169` / `4cdf776` / `13ae14b` / `9bc3f22` + post-review fix `be5eced`. Validator clean (0 errors). **Remaining:** Phase B content authoring (Tasks 27-35, human-in-the-loop) → Task 36 (pool coverage validator, runs after content) → Task 41 (loc sync) / 42 (delete legacy `src/Features/Orders/`) / 43 (smoke). Phase C/D (Tasks 44-60) upcoming. Separately: `StoryletCatalog` / `QualityCatalog` per-item parse-fail gap patched (`9c2ce23`), surfaced during Spec 2 loader audit. Plan: [docs/superpowers/plans/2026-04-20-orders-surface.md](docs/superpowers/plans/2026-04-20-orders-surface.md).
