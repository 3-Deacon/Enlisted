...
- **Five-plan integration roadmap (2026-04-21)** — the Spec 2 Orders Surface remainder and the Campaign Intelligence Backbone were merged into one integrated loop. Integration spec: [docs/superpowers/specs/2026-04-21-plans-integration-design.md](docs/superpowers/specs/2026-04-21-plans-integration-design.md). Design spec (truth layer): [docs/superpowers/specs/2026-04-21-enlisted-campaign-intelligence-design.md](docs/superpowers/specs/2026-04-21-enlisted-campaign-intelligence-design.md).
  ...
  - **Plan 5** — Career Loop Closure (path crossroads + culture/trait overlays + debug hotkeys + save-load + final playtest). Plan at [docs/superpowers/plans/2026-04-21-career-loop-closure.md](docs/superpowers/plans/2026-04-21-career-loop-closure.md). 🟡 **Half A partial** — Tasks T1-T7 shipped locally on `development` 2026-04-22 (commits `4f66604` → `88e7486`, 8 commits; not yet pushed to origin). Ships the `commit_path` / `resist_path` effect primitives, `QualityStore.SetDirect` back-door for read-only indicators, `PathIds` shared-helper, `PathScorer.BumpPath` resist 0.5× bias, `PathCrossroadsBehavior` firing `path_crossroads_<path>_t{4,6,9}` Modals on static `EnlistmentBehavior.OnTierChanged`, 15 authored crossroads storylets, and 5 path T7+ variant files (scout+escort archetypes × 5 paths = 30 storylets, minimum-viable Half A scope). T9 scope was relaxed from "each path × each of 10 archetypes" to "each path has ≥1 T7+ variant" — Half B polish expands the archetype matrix. **Pending:** T8 (`prior_service_*` flag-emission grep-verify — likely no-op since `EnlistmentLifecycleListener.cs:100-101` already emits), T9 (Phase 15 validator rewrite — stub at `validate_content.py:2952` currently gates on legacy prefix `crossroads_` with bands `t3_to_t4`, must be rewritten to `path_crossroads_*_t{4,6,9}`), T10 (in-game smoke). Plus Half B (T11-T22: culture/trait overlays, debug hotkeys, save-load test, playtest scenarios A-G, verification doc).
