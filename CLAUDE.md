...
- **Five-plan integration roadmap (2026-04-22)** — Plans 1-4 ✅ shipped + smoked on `development`; Plan 5 🟡 code-shipped, in-game smoke pending. Integration spec: [docs/superpowers/specs/2026-04-21-plans-integration-design.md](docs/superpowers/specs/2026-04-21-plans-integration-design.md). Design spec (truth layer): [docs/superpowers/specs/2026-04-21-enlisted-campaign-intelligence-design.md](docs/superpowers/specs/2026-04-21-enlisted-campaign-intelligence-design.md). Save-definer offsets claimed: 48 (intelligence snapshot), 49 (signal record), 50 (duty cooldown).
...
**TaleWorlds APIs:** NEVER use Context7, web search, or training knowledge. Always use the local decompile tree — it is the only authoritative reference for the Bannerlord API surface the user has installed. Location depends on environment: `../Decompile/` on Windows (sibling of repo root, external to git) or `./Decompile/` on WSL (inside repo, gitignored). Regenerate per AGENTS.md Quick Commands when the install is updated or the directory is missing.
...
### Shell & PATH — depends on environment

This repo is developed in both **Windows Git Bash** and **WSL Linux**. Detect with `uname -s` (`MINGW64_NT-*` = Git Bash, `Linux` = WSL). The quirks differ.

**Windows Git Bash:**

- Use Unix paths (`/dev/null`, forward slashes), not `NUL` or backslashes
- Thin shim — `cat`, `head`, `tail`, `grep`, `file`, `which` not on PATH. Use Grep / Read / Write tools instead
- `dotnet`, `git`, `python` not on PATH. Prepend: `export PATH="/c/Program Files/dotnet:/c/Program Files/Git/cmd:$PATH"`. Python at `/c/Python313/python.exe`

**WSL Linux:**

- Standard Unix tools all work natively (no shim)
- `dotnet`, `git`, `python3` already on PATH (`/usr/bin/...`)
- Windows `.exe` interop is broken in this environment — `cmd.exe`, `powershell.exe`, `ssh.exe` all return "Invalid argument". Don't invoke Windows binaries from WSL. For GitHub, this repo is wired with HTTPS + `gh` credential helper (see `git config --local --get-regexp '^(credential|url|remote)'`)
- The game install + Windows checkout are still accessible via `/mnt/c/...` mounts for file reads. The canonical Windows checkout lives at `/mnt/c/dev/Enlisted/Enlisted` (active dev branch) and Steam Bannerlord at `/mnt/c/Program Files (x86)/Steam/steamapps/common/Mount & Blade II Bannerlord`

### Build & commit

- AGENTS.md's build form `/p:Platform=x64` trips bash's argument parser when the config has a space. From either bash use: `dotnet build Enlisted.sln -c 'Enlisted RETAIL' -p:Platform=x64`
- Multi-line commit messages:
  - **WSL Linux:** `git commit -m "$(cat <<'EOF' ... EOF)"` heredoc works fine
  - **Windows Git Bash:** no `cat` heredoc. Write the message to a temp file (e.g. `/c/Users/<you>/commit.txt`) and pass with `git commit -F <file>`. **Known footgun:** the Write tool errors if you skip `Read` first, and when it errors, the OLD content from a prior task's message stays in place — `git commit -F` then ships a misnamed commit with the previous task's subject. Always `Read` the temp file before `Write`, every time
- Another AI session may be editing files concurrently. Stage with `git add <path>`, never `git add -A` — in-flight edits belong to the other session

### File handling

- Line endings: `.gitattributes` enforces CRLF on `.cs` / `.csproj` / `.sln` / `.ps1`; everything else `text=auto`
  - **Windows Git Bash, newly created files:** run `powershell.exe -NoProfile -ExecutionPolicy Bypass -File Tools/normalize_crlf.ps1 -Path <file>` exactly once. **Known bug:** the script prepends a UTF-8 BOM unconditionally, so a second run on the same file creates a double-BOM defect
  - **WSL Linux:** `powershell.exe` does not work (interop broken). Rely on `.gitattributes` at commit time — `core.autocrlf=input` in your `~/.gitconfig` plus the per-file `.gitattributes` rules handle it. Or run `unix2dos <file>` if you have `dos2unix` installed
  - **Edits to existing `.cs` files (either env):** rely on `.gitattributes` at commit time — don't run any normalization
