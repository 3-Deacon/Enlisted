...
- **Spec 2 (Orders Surface)** — Phase A in progress. Tasks 1-11 of 60 shipped (`2f64137` → `36a32c9`): EnlistmentBehavior events, Storylet schema extension, 12 new effect primitives, CultureTroopTreeHelper + CombatClassResolver extractions, OrderActivity + NamedOrderState with arc-reconstruction hook, save-definer offsets 46/47/84/85, DutyProfileId enum, DutyProfileSelector. Phase A remaining: Tasks 12-17 (DutyProfileBehavior, LordStateListener, NamedOrderArcRuntime, DailyDriftApplicator, OrdersNewsFeedThrottle, PathScorer, Phase A smoke). Plan: [docs/superpowers/plans/2026-04-20-orders-surface.md](docs/superpowers/plans/2026-04-20-orders-surface.md).
