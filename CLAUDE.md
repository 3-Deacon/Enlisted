...
- **Five-plan integration roadmap (2026-04-21)** — the Spec 2 Orders Surface remainder and the Campaign Intelligence Backbone were merged into one integrated loop. Integration spec: [docs/superpowers/specs/2026-04-21-plans-integration-design.md](docs/superpowers/specs/2026-04-21-plans-integration-design.md). Design spec (truth layer): [docs/superpowers/specs/2026-04-21-enlisted-campaign-intelligence-design.md](docs/superpowers/specs/2026-04-21-enlisted-campaign-intelligence-design.md).
  ...
  - **Plan 5** — Career Loop Closure (path crossroads + culture/trait overlays + debug hotkeys + save-load + final playtest). Plan at [docs/superpowers/plans/2026-04-21-career-loop-closure.md](docs/superpowers/plans/2026-04-21-career-loop-closure.md). 🟡 **Half A partial** — Tasks T1-T9 shipped locally on `development` 2026-04-22 (commits `4f66604` → `e35f445`, 10 commits; not yet pushed to origin). Ships the `commit_path` / `resist_path` effect primitives, `QualityStore.SetDirect` back-door for read-only indicators, `PathIds` shared-helper, `PathScorer.BumpPath` resist 0.5× bias, `PathCrossroadsBehavior` firing `path_crossroads_<path>_t{4,6,9}` Modals on static `EnlistmentBehavior.OnTierChanged`, 15 authored crossroads storylets, 5 path T7+ variant files (scout+escort archetypes × 5 paths = 30 storylets, minimum-viable Half A scope), and `validate_content.py` Phase 15 rewritten from stub to full enforcement (15-combo assertion + per-path has_any_t7_variant check on `flag:committed_path_<path>` trigger). T8 was verified no-op (`prior_service_culture_<id>` + `prior_service_rank_<N>` already emitted at `EnlistmentLifecycleListener.cs:100-101`). T9 scope was relaxed from "each path × each of 10 archetypes" to "each path has ≥1 T7+ variant" — Half B polish expands the archetype matrix. **Pending:** T10 (in-game smoke — human operator). Plus Half B (T11-T22: culture/trait overlays, debug hotkeys, save-load test, playtest scenarios A-G, verification doc).
