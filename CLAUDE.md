...
- **Spec 2 (Orders Surface)** — Phase A complete; Phase B §13.1 migrations complete; Task 19f (deferred ShowOrdersMenu site) shipped as `0eed684`. Tasks 1-17 shipped on `development` (commits `2f64137` → `b8039cf`): EnlistmentBehavior events, Storylet schema extension, 12 new effect primitives, CultureTroopTreeHelper + CombatClassResolver extractions, OrderActivity + NamedOrderState with arc-reconstruction, save-definer offsets 46/47/85 (FormationClass is vanilla-registered — see pitfall below), DutyProfileId enum + DutyProfileSelector + DutyProfileBehavior (12-hour transition-beat hysteresis), EnlistmentLifecycleListener (startup subscribes to `OnEnlisted` + `OnSessionLaunchedEvent`; teardown narrowed from LordStateListener — see appendix), NamedOrderArcRuntime (save-load arc splice), DailyDriftApplicator + OrdersNewsFeedThrottle (skill-XP drift with wall-clock gate; TryClaim rejects at SpeedUpMultiplier > 4f by design), PathScorer (path specialization scoring), in-game smoke verified (PATH heartbeat, DUTYPROFILE profile_changed, DRIFT daily_tick). Phase B §13.1 migrated 13 OrderManager consumer sites to `OrderDisplayHelper` across `EnlistedMenuBehavior` (5), `EnlistedNewsBehavior` (4), `ForecastGenerator` (2), `CampOpportunityGenerator` (1), plus 1 ShowOrdersMenu site (now covered by `0eed684`). **Phase B content started:** Task 25 (`duty_profiles.json`, `85d8b35`) and Task 26 (`LootPoolResolver` + `loot_pools.json` + EffectExecutor rewire, `90bef6a`) shipped — see appendix for four API corrections applied during Task 26 execution. **Phase B validator rails shipped:** Tasks 37-40 added validator phases 14 (arc integrity), 15 (crossroads stub — skips until any `crossroads_*` storylet is authored), 16 (loot pool sanity), and 17 (per-option rate caps + `committed_path` added to `_READ_ONLY_QUALITY_IDS`) in `Tools/Validation/validate_content.py` — commits `55e7169` / `4cdf776` / `13ae14b` / `9bc3f22`, plus `be5eced` for post-review fixes (long-form `relation_change` coverage, defensive non-dict guards on `arc` and pool entries, source-file-path threading so error messages point at the authored JSON). Validator runs clean against current corpus (0 errors / 20 warnings / 91 info). Remaining Phase B: content authoring (Tasks 27-35) — human-in-the-loop per storylet batch — then Task 36 (Phase 13 pool coverage validator, designed to run AFTER content to close gaps it reveals), Task 41 (localization sync), Task 42 (delete old `src/Features/Orders/` subtree), Task 43 (Phase B smoke). Phase C/D (44-60) still upcoming. Separately: per-item parse-fail gap in `StoryletCatalog` / `QualityCatalog` patched (`9c2ce23`), not a Spec 2 task but surfaced during Spec 2 loader audit. Plan: [docs/superpowers/plans/2026-04-20-orders-surface.md](docs/superpowers/plans/2026-04-20-orders-surface.md).
