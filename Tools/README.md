...
**Common categories:** `Interface`, `Battle`, `Enlistment`, `DUTYPROFILE`, `PATH`, `Content`, `Equipment`, `Orchestrator`
...
| `sync_event_strings.py` | Extract string IDs from JSON and sync to XML localization. Scans two schemas: legacy Event schema (Events/, Decisions/ — separate `titleId`+`title` pairs) and storylet inline `{=key}Fallback` (Storylets/ — embedded in `title`/`setup`/`options[].text`/`options[].tooltip` fields). Missing XML entries don't break the game (fallback text is used), but they must exist for translators. |
