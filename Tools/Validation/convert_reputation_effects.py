    ...
    if "options" in event:
        ...
        for option in event["options"]:
            ...
            if "effects_success" in option:
                new_success_effects, success_effects_changed = convert_effects(
                    option["effects_success"]
                )
                if success_effects_changed:
                    new_option["effects_success"] = new_success_effects
                    changed = True
            if "effects_failure" in option:
                new_failure_effects, failure_effects_changed = convert_effects(
                    option["effects_failure"]
                )
...
def main():
    ...
    for json_file in sorted(json_files):
        ...
        if file_changed:
            ...
            print(
                f"{'[DRY RUN] ' if dry_run else ''}Updated {rel_path}: {events_changed} events changed"
            )
