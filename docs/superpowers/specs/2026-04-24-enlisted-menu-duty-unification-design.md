...
**Status:** Draft v5 (2026-04-25). Consolidates the menu restructure, the news-unification body layout, and the Routine→Duty reframe into one surface design. Supersedes parts of three prior docs (see "Relation to prior specs"). V2 corrected eight factual errors from adversarial review; v3 added two blocking architecture fixes; v4 corrects the gear-progression model — the QM is already a shop in the existing codebase, so Duty changes shop *stock* rather than auto-swapping equipment. V5 records the legacy decisions/orchestrator/Camp* retirement that pre-empted parts of Plan A. See Changelog at the bottom.

**Pre-existing retirement (2026-04-25).** The legacy decisions surface, `ContentOrchestrator`, `CampOpportunityGenerator`/`CampScheduleManager`/`CampRoutineProcessor`, and 8 supporting POCO models were retired in a separate cleanup pass before Plan A starts. Concretely: the `enlisted_decisions_header` accordion + 5 decision slots are gone from `enlisted_status`; `BuildScheduleForecast` and the schedule-driven branches of `BuildUpcomingSection` / `BuildBriefPlayerForecast` are gutted; ~830 lines of decision-handling methods (`GetCurrentDecisions`, `OnDecisionSelected`, `ConvertDecisionToEvent`, `ConvertScheduledToDecisionAvailability`, etc.) are deleted. Two simulation pieces were re-homed: illness-onset modal firing → `CompanySimulationBehavior.CheckIllnessOnset` (Phase 7 daily tick); quiet-day predicate → `GlobalEventPacer.ComputeIsQuietDay` self-tick. Plan A's surgery list shrinks accordingly — the menu shell already lacks the decisions accordion and Equipment option; Plan A authors only need to deliver the new DISPATCHES + PROSPECTS + COMPANY NEWS + STATUS sections plus the Camp options strip (Duty / Quartermaster / Records / Companions / Retinue / Back).

## Problem statement

Today's enlisted menu has three layered problems:

1. **Body flood.** `enlisted_status` body concatenates 15+ short lines across three named sections (KINGDOM REPORTS / COMPANY REPORTS / PLAYER STATUS — see `EnlistedMenuBehavior.cs:3315-3347`). The Camp hub body adds five more sections (SINCE LAST MUSTER / UPCOMING / RECENT ACTIVITY / STATUS + smaller variants — see `EnlistedMenuBehavior.cs:1595-1628`). The display refresh path runs frequently under fast-forward, and some content can rebuild often enough to churn while the player is still reading the section above it.
...
1. **Battle formation** — Combat Duties set the player's formation slot (Infantry / Ranged / Cavalry / Horse Archer). **This requires new wiring with a gear-awareness rule.** Today `CombatClassResolver.Resolve(hero)` at `CombatClassResolver.cs:25-40` reads `character.IsRanged` / `character.IsMounted` (equipment-derived); `EnlistedFormationAssignmentBehavior.cs:466-470` then assigns the player to the formation that class maps to. Phase 1 changes `CombatClassResolver` to query a new `DutyManager.EffectiveCombatDuty` property first for the player, falling back to equipment-derived class when no Duty is set or for non-player heroes. `EnlistedFormationAssignmentBehavior` keeps its in-battle assignment role; only the class-lookup source changes.

   **`EffectiveCombatDuty` — the gear-awareness rule.** `DutyManager` exposes two properties:
...
| `enlisted_decisions_header` accordion + 5 slots | — | **Already retired 2026-04-25.** Plan A inherits the gap. |
| `ContentOrchestrator` + `CampOpportunityGenerator` + `CampScheduleManager` + `CampRoutineProcessor` | — | **Already retired 2026-04-25.** Illness-onset firing re-homed to `CompanySimulationBehavior`; quiet-day to `GlobalEventPacer`. |
...
4. **F** — Gear reissue via Quartermaster. Must land before D so `EffectiveCombatDuty` has working gear-alignment to check; without F, `EffectiveCombatDuty` would always fall back to equipment-derived (no Duty would ever drive formation).
5. **D** — Combat Duty → formation. First real in-battle consequence of picking a Duty (after gear reissue catches up).
...
Ship-order rationale: F → D (not D → F) because Plan D's `EffectiveCombatDuty` reads equipment to decide whether the chosen Duty is gear-aligned. If F hasn't shipped, gear never matches a Cavalry/HA choice, so `EffectiveCombatDuty` always returns the equipment-derived class and Plan D is a no-op for the player.
...
- **v3 (2026-04-24):** two blocking fixes + one polish from a second round of adversarial review, re-verified against primary sources:
...
- **v5 (2026-04-25):** records the pre-Plan-A retirement of the legacy decisions / orchestrator / Camp* simulation cluster. Pre-existing-retirement note added at the top; consolidation table gains two rows (decisions accordion + 5 slots; ContentOrchestrator + CampOpportunityGenerator + CampScheduleManager + CampRoutineProcessor); Plan A row in Implementation plans flags the pre-work. Two simulation pieces re-homed (illness-onset → `CompanySimulationBehavior.CheckIllnessOnset`; quiet-day predicate → `GlobalEventPacer.ComputeIsQuietDay`). Plan A's scope unchanged on the *building* side — it still delivers the new section layout + Camp options strip — but the surgery side shrinks since the accordion + slots + schedule-forecast branches no longer exist to delete.
