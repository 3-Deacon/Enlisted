...
**Date:** 2026-04-19
**Status:** Drafted 2026-04-19; revised same day after adversarial review caught six integration gaps (activity-type loader, Storylet→InteractiveEvent adapter, menu slot-bank pattern, party-identity gate, OrchestratorOverride scope, effect schema corrections). **Phase A (substrate, Tasks 1–5) complete 2026-04-19** on `development` (commits `fc93285` → `24b7e50`). **Phase B (HomeActivity + save + triggers + lifecycle starter, Tasks 6–9) complete 2026-04-20** on `development` (commits `e4b6806` → `a48983c`). **Phase C (menu integration + smoke, Tasks 10–12) complete 2026-04-20** on `development` (commits `18b4021` → `19391a4`). Phase D (50 storylets + localization, Tasks 13–18) and Phase E (debug hotkeys + validator + acceptance, Tasks 19–21) not yet started. Depends on Spec 0 (Storylet Backbone, landed `45b38bf`). Implementation plan: [docs/superpowers/plans/2026-04-19-enlisted-home-surface.md](../../superpowers/plans/2026-04-19-enlisted-home-surface.md).
...
| `Ctrl+Shift+T` | Dump trigger evaluation table (intent slots + arrive/settle pools) to session log |
