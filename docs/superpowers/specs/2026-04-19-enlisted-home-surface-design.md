...
**Date:** 2026-04-19
**Status:** **Implementation complete 2026-04-20** on `development`. Full commit trail `fc93285` → `b563834`. Phases A (substrate, `fc93285`→`24b7e50`), B (HomeActivity + save + triggers + starter, `e4b6806`→`a48983c`), C (menu integration + smoke, `18b4021`→`19391a4`), D (50 storylets + localization, `8b96294`→`d9bcf8b`), E (debug hotkeys + validator + acceptance smoke, `1d0fb52`→`b563834`). Task 21 Scenarios A-F verified in-game; three followups carried to Spec 2 backlog (csproj ModuleData deploy rule, Ctrl+Shift+T hotkey rebind, AdvanceHomeToEvening auto-phase skip). Drafted 2026-04-19; revised same day after adversarial review caught six integration gaps (activity-type loader, Storylet→InteractiveEvent adapter, menu slot-bank pattern, party-identity gate, OrchestratorOverride scope, effect schema corrections). Depends on Spec 0 (Storylet Backbone, landed `45b38bf`). Implementation plan: [docs/superpowers/plans/2026-04-19-enlisted-home-surface.md](../../superpowers/plans/2026-04-19-enlisted-home-surface.md).
