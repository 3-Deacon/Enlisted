...
**Date:** 2026-04-18
**Status:** In progress — Phases 1 & 2 complete and build-green (2026-04-18). The Director now routes live: Modal candidates fire through pacing floors (in-game day + wall-clock + category cooldown), floor-blocked interactives buffer in a deferred list, observational items land in `_personalFeed` via `EnlistedNewsBehavior.AddPersonalDispatch`, a `DailyTick` listener fires a quiet-stretch event after 14 silent days, `enlisted_status` carries a Headlines drilldown, and the muster intro digest filters by severity. **Phase 3 (Tasks 17–30) is pending:** 14 production `EventDeliveryManager.Instance.QueueEvent(...)` call sites still bypass the Director, and the deferred-interactive retry queue is not yet wired. Phase 4 (Tasks 31–32) covers smoke tests + BLUEPRINT / INDEX cross-links. Implementation plan at [docs/superpowers/plans/2026-04-18-event-pacing.md](../plans/2026-04-18-event-pacing.md) — see its top section for commit-by-commit progress.
