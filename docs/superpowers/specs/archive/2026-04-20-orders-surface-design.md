...
- **No rewrite of `StoryDirector` pacing.** Modal events (named-order accept, transitions, crossroads, lord-state edge cases) inherit the existing 5-day in-game floor + 60s wall-clock floor + per-category cooldown unchanged (per AGENTS.md Critical Rule #10). Spec 2 adds a single thin pre-filter (`OrdersNewsFeedThrottle`, see §5) for its own non-modal NewsFeed emissions only — that pathway is the gap StoryDirector doesn't cover.
...
Plus a thin pre-filter and extensions to existing infrastructure:

- `src/Features/Activities/Orders/OrdersNewsFeedThrottle.cs` — **new, ~30 lines.** Single-category wall-clock floor on Spec 2's non-modal `StoryDirector.EmitCandidate(...)` calls. `TryClaim()` returns false if either (a) the wall-clock floor (default 20 real seconds) hasn't elapsed since the last successful claim, or (b) `Campaign.Current.SpeedUpMultiplier > 4f` (player is in mod-enabled "ludicrous speed" — a deliberate skip, not reading). Drop on overflow; no queueing. Modal emissions bypass entirely (StoryDirector already paces them). Sole consumers: `DutyProfileBehavior` (ambient profile-storylet emission) and `DailyDriftApplicator` (drift-summary emission).
...
**R10 — Wall-clock cadence under fast-forward (the "shouldn't be annoying" risk).** Bannerlord runs at variable speed (1×–4× default, mods push higher). At 4× a game day = 6 real minutes; at 16× a game day = 90 real seconds. Naively scheduling "fire daily floor entry on `DailyTickEvent`" + "fire ambient profile storylet hourly" produces a wall of news-feed entries during fast-forward — defeating the "background sim" framing by becoming the foreground noise. **Investigation against the v1.3.13 decompile confirmed native game does not solve this**: every `CampaignBehaviorBase` rolls its own cooldown using game-time deltas, no global throttle, no fast-forward suppression, no notification queue. The summary pattern (`CharacterRelationCampaignBehavior.cs:43-79`: accumulate inside a loop, surface ONE summary notification) is the closest native idiom and the right model to borrow. Spec 0's `StoryDirector` already does the wall-clock layer for Modal events (5-day in-game floor + 60s wall-clock floor + per-category cooldown per AGENTS.md Critical Rule #10), but non-modal NewsFeed entries flow through to the accordion without a wall-clock layer — that's the gap Spec 2's emissions hit. **Mitigation:** §5's `OrdersNewsFeedThrottle` adds a single 20-second wall-clock floor on Spec 2's non-modal emissions and suppresses non-modals entirely above 4× speed. `DailyDriftApplicator` adopts the summary pattern: accumulate skill XP silently in `OrderActivity.DriftPendingXp`, flush to one summary news-feed entry per ~90 real seconds (whenever the throttle next allows), clear. Result: at 1× the player sees rich texture; at 4× they see a measured trickle; at 16× ambient + drift are silent and only Modal interrupts (named-order accept, transition, crossroads, lord-state) surface. Aligns with the design intent: orders are background flavor, not foreground spam.
