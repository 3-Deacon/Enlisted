...
| Author or inspect current content | [Features/Content/storylet-backbone.md](Features/Content/storylet-backbone.md) |
...
| **Menu + Duty Unification** 📋 Current design | [design](superpowers/specs/2026-04-24-enlisted-menu-duty-unification-design.md) / [agency-news plan](superpowers/plans/2026-04-23-agency-news-status-integration.md) | Current unifying design for menu/news/duty work. Supersedes the 2026-04-23 section names (`UPCOMING` -> `PROSPECTS`, `YOU` -> `STATUS`) and reframes unshipped service stances as player Duty. The agency-news plan remains actionable after enum-name amendments (`ServiceStance` -> `Duty`, `Upcoming` -> `Prospects`). |
| **CK3 Wanderer Mechanics — 7-plan family** 🟡 Plans 1-3 code-shipped (smoke pending); 4-7 not started | [spec v6](superpowers/specs/2026-04-24-ck3-wanderer-systems-analysis.md) / [architecture brief (locked)](architecture/ck3-wanderer-architecture-brief.md) / [Plan 1](superpowers/plans/2026-04-24-ck3-wanderer-architecture-foundation.md) / [Plan 1 verification](superpowers/plans/2026-04-24-ck3-wanderer-architecture-foundation-verification.md) / [Plan 2](superpowers/plans/2026-04-24-ck3-wanderer-companion-substrate.md) / [Plan 2 verification](superpowers/plans/2026-04-24-ck3-wanderer-companion-substrate-verification.md) / [Plan 3](superpowers/plans/2026-04-24-ck3-wanderer-rank-ceremony-arc.md) / [Plan 3 verification](superpowers/plans/2026-04-24-ck3-wanderer-rank-ceremony-arc-verification.md) / [Plan 4 draft](superpowers/plans/2026-04-24-ck3-wanderer-officer-trajectory.md) / [Plan 5 draft](superpowers/plans/2026-04-24-ck3-wanderer-endeavor-system.md) / [Plan 6 draft](superpowers/plans/2026-04-24-ck3-wanderer-roll-of-patrons.md) / [Plan 7 draft](superpowers/plans/2026-04-24-ck3-wanderer-polish-smoke-tuning.md) | Importing the CK3 wanderer career arc onto Bannerlord's enlistment substrate. Plan 1 (Architecture Foundation, commit `aa3ef16`) ships save-definer class offsets 54-58 + enum 84 (`PatronRoll` / `PatronEntry` / `ContractActivity` / `EndeavorActivity` / `LifestyleUnlockStore` / `FavorKind`), four hosting `CampaignBehaviorBase` shells, `ModalEventBuilder` helper centralizing the `Storylet → BuildModal → EmitCandidate` pipeline, and the all-tier stay-back gate fix in `EnlistedFormationAssignmentBehavior`. Plan 2 (Companion Substrate, commits `57941d2` → `4dfe719`) spawns six wanderer-mechanics companions (Sergeant T1, Field Medic + Pathfinder T3, Veteran T5, QM Officer + Junior Officer T7) with predicate-based troop selection, three archetype variants per companion, ~120 dialog catalog nodes (full token-interpolation contract per AGENTS.md pitfall #23), Camp menu Talk-to surface, OnHeroKilled cleanup, per-Lord vs per-player lifecycle. Plan 3 (Rank-Ceremony Arc, code shipped 2026-04-26) ships five character-defining ceremony modals at retained tier transitions (T1→T2, T2→T3, T4→T5, T6→T7 commission, T7→T8) — three colliding tiers (newTier 4/6/9) deliberately skipped per Lock 1 because `PathCrossroadsBehavior` already fires modals there. 20 storylets × 64 options across base + 3 cultural variants per ceremony, 44 new scripted effects (10 trait drift + 34 witness reactions × 7 archetypes including `lord`), 188 inline loc-keys synced. Choice memory rides on `FlagStore` bool keys (`ceremony_fired_t{N}` dedup + `ceremony_choice_t{N}_<choice_id>` per pick); zero new save-definer offsets. Single `OnTierChanged` hook covers all 3 T6→T7 promotion paths. Living references: [companion-archetype-catalog.md](Features/Companions/companion-archetype-catalog.md), [ceremony-storylet-schema.md](Features/Ceremonies/ceremony-storylet-schema.md), [ceremony-flag-conventions.md](Features/Ceremonies/ceremony-flag-conventions.md). |
| **Companion Archetype Catalog (Plan 2)** | [companion-archetype-catalog.md](Features/Companions/companion-archetype-catalog.md) | Schema reference for `ModuleData/Enlisted/Companions/archetype_catalog.json`. Predicate-based troop selection (tier + formation + occupation), five-trait whitelist (Valor/Mercy/Honor/Calculating/Generosity), three-archetype-per-companion contract, civilian style stub. Loaded at session launch by `Enlisted.Features.Companions.CompanionSpawnFactory`. |
| **Rank-Ceremony Arc (Plan 3)** 🟡 Code shipped, smoke pending | [Ceremony storylet schema](Features/Ceremonies/ceremony-storylet-schema.md) / [Ceremony flag conventions](Features/Ceremonies/ceremony-flag-conventions.md) / [plan](superpowers/plans/2026-04-24-ck3-wanderer-rank-ceremony-arc.md) / [verification](superpowers/plans/2026-04-24-ck3-wanderer-rank-ceremony-arc-verification.md) | Five character-defining ceremony modals at tier-up moments (T1→T2, T2→T3, T4→T5, T6→T7 commission, T7→T8). Each ceremony asks a question, drifts vanilla traits (Mercy/Valor/Honor/Generosity/Calculating) by ±1, drifts companion-witness relations via `ChangeRelationAction.ApplyPlayerRelation`, and writes choice-memory flags downstream plans 4-7 read to flavor content. `RankCeremonyBehavior.OnTierChanged` is the single hook; `CeremonyProvider.FireCeremonyForTier` is the public accessor. |
...
**Location:** `Features/Companions/`

| Document | Topic | Status |
| --- | --- | --- |
| [companion-archetype-catalog.md](Features/Companions/companion-archetype-catalog.md) | Schema reference for `ModuleData/Enlisted/Companions/archetype_catalog.json`. Six wanderer-mechanics companions (Sergeant T1, Field Medic + Pathfinder T3, Veteran T5, QM Officer + Junior Officer T7), three archetype variants each, predicate-based troop selection. Read by `CompanionSpawnFactory`. | ✅ Current |

### Ceremonies (Plan 3)

**Location:** `Features/Ceremonies/`

| Document | Topic | Status |
| --- | --- | --- |
| [ceremony-storylet-schema.md](Features/Ceremonies/ceremony-storylet-schema.md) | Ceremony storylet schema reference: 5 retained tier transitions (T1→T2, T2→T3, T4→T5, T6→T7 commission, T7→T8), 4-storylet shape per file (base + 3 cultural variants), token interpolation contract, witness reaction conventions, scripted-effect ID families. | ✅ Current |
| [ceremony-flag-conventions.md](Features/Ceremonies/ceremony-flag-conventions.md) | Choice memory flag-key conventions: `ceremony_fired_t{N}` dedup + `ceremony_choice_t{N}_<choice_id>` per option (one bool per choice — `FlagStore` is bool-only). Plans 4-7 read these flags. | ✅ Current |
...
| [storylet-backbone.md](Features/Content/storylet-backbone.md) | Storylet backbone (Spec 0) — content layer beneath StoryDirector. Storylets, qualities, flags, activities, scripted effects (including Plan 3 ceremony families). Living reference for authoring against the substrate. | ✅ Current |
...
| [orders-content.md](Features/Content/orders-content.md) | Retired legacy Orders catalog. The old `src/Features/Orders/` subsystem was replaced by `OrderActivity` + storylet pools; current authoring belongs in [storylet-backbone.md](Features/Content/storylet-backbone.md), [career-loop.md](Features/Content/career-loop.md), and the five-plan roadmap. Historical plan: [archived orders surface](superpowers/plans/archive/2026-04-20-orders-surface.md). | 📦 Archived |
...
| [news-reporting-system.md](Features/UI/news-reporting-system.md) | News feeds and reporting: kingdom/personal feeds, `HEADLINES`, camp hub recaps, typed dispatch fields (`Tier` / `Beats` / `Body`), modal queue persistence, combat-log ownership boundary, and current `development` status. Agency typed routing remains branch-scoped until merged. | ✅ Current |
...
| Document | Purpose | Status |
| --- | --- | --- |
| [README.md](Features/Content/README.md) | Content system overview. Current authoring routes through storylets, activities, qualities, flags, scripted effects, and the five-plan roadmap; legacy Orders content is archived for historical reference. | ✅ Current |
