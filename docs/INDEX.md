...
| **Spec 2 Orders Surface** 🛠 Phase A in progress 2026-04-20 | [design](superpowers/specs/2026-04-20-orders-surface-design.md) / [plan](superpowers/plans/2026-04-20-orders-surface.md) | Replaces the 11-file `src/Features/Orders/` subsystem with a single `OrderActivity` on the storylet backbone. Real Bannerlord rewards (focus / attribute / trait / renown / relations / native loot), six duty profiles with hysteresis, ten named-order arcs, five-path emergent specialization with T3/T5/T7 crossroads. Tasks 1-11 of 60 shipped: `EnlistmentBehavior` events, Storylet schema, 12 effect primitives, `CultureTroopTreeHelper` + `CombatClassResolver` extractions, `OrderActivity` + `NamedOrderState` with arc-reconstruction hook, save offsets 46/47/84/85, `DutyProfileId` + `DutyProfileSelector`. |
...
| [orders-content.md](Features/Content/orders-content.md) | Complete catalog of all 17 implemented orders with event pools, duration, skills, fatigue, injury risk, and sea/land filtering. Live reference until Spec 2 replaces the subsystem — **Spec 2 Phase A underway** (Tasks 1-11 of 60 shipped; [plan](superpowers/plans/2026-04-20-orders-surface.md)). | ✅ Current |
