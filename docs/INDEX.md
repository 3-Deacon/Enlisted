...
| **Spec 2 Orders Surface** 🛠 Phase A code complete 2026-04-20 | [design](superpowers/specs/2026-04-20-orders-surface-design.md) / [plan](superpowers/plans/2026-04-20-orders-surface.md) | Replaces the 11-file `src/Features/Orders/` subsystem with a single `OrderActivity` on the storylet backbone. Real Bannerlord rewards (focus / attribute / trait / renown / relations / native loot), six duty profiles with hysteresis, ten named-order arcs, five-path emergent specialization with T3/T5/T7 crossroads. Tasks 1-16 of 60 shipped (Task 17 user-driven in-game smoke pending): `EnlistmentBehavior` events, Storylet schema, 12 effect primitives, `CultureTroopTreeHelper` + `CombatClassResolver` extractions, `OrderActivity` + `NamedOrderState` with arc-reconstruction, save offsets 46/47/84/85, `DutyProfileId` + `DutyProfileSelector` + `DutyProfileBehavior`, `EnlistmentLifecycleListener` (narrowed), `NamedOrderArcRuntime`, `DailyDriftApplicator` + `OrdersNewsFeedThrottle`, `PathScorer`. |
